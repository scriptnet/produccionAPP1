...
class RestaurantLogic
{
    ...
    public static function get_restaurants($limit = 10, $offset = 1, $zone_id, $filter)
    {
        ...
        ->with('discount')->where('zone_id', $zone_id)
    ...
    {
        return Restaurant::with('discount', 'campaigns')->active()->whereId($restaurant_id)->first();
    }
    ...
    {
        $key = explode(' ', $name);
        $paginator = Restaurant::with('discount')->where('zone_id', $zone_id)->weekday()->where(function ($q) use ($key) {
