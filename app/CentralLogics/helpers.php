...
class Helpers
{
    ...
    public static function send_order_notification($order)
    {
        ...
        try {
            ...
            if(!$order->scheduled && $order->order_status == 'confirmed' && $order->order_type != 'take_away' && config('order_confirmation_model') == 'deliveryman')
            {
                $data = [
                    'title' =>trans('messages.order_push_title'),
                    'description' => trans('messages.new_order_push_description'),
            ...
            else if(!$order->scheduled && $order->order_status == 'confirmed' && $order->payment_method != 'cash_on_delivery')
            {
                $data = [
                    'title' =>trans('messages.order_push_title'),
                    'description' => trans('messages.new_order_push_description'),
